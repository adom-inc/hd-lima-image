...
description: Test/preview the Hydrogen golden arm64 Rosetta-hybrid rootfs by importing it into a DISPOSABLE systemd-nspawn machine inside a throwaway Lima VM on the user's Mac (real systemd PID 1 → exercises the timer + workspace-updater daemon AND gives a real code-server to pup). Use when the user says "test the golden image", "run the golden image", "serve the rootfs", "let me see the golden code-server", "verify the image config", or reports something wrong/missing in a golden image build. Every confirmed gap gets added to image/bake-hydrogen-setup.sh AND a smoke assertion in scripts/build-rootfs.sh — that is the rule.
---

# Golden image test — disposable Lima machine (macOS)

Validate a built/released golden image the way Hydrogen actually runs it: as a
...
throwaway `lima-golden-test` VM, so the user's `hydrogen-builder` + `Adom-Workspace`
are untouched.

```bash
# 0. Throwaway Lima VM (vz + Rosetta), separate from Hydrogen's hydrogen-builder:
...
limactl shell lima-golden-test -- bash -lc \
  'curl -fL "https://github.com/adom-inc/hydrogen-lima-image/releases/download/vN/adom-golden-vN-arm64.tar.gz" -o /tmp/g.tar.gz'
...
A 'adom-bridge --version'                          # x86-64 binary running under Rosetta
...
- The rule: every confirmed gap → a new step in `image/bake-hydrogen-setup.sh` AND a
