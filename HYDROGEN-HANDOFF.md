...
Built from `adom-inc/hydrogen-lima-image` (public repo), hosted as a GitHub Release
asset. macOS sibling of `adom-inc/hd-wsl2-image` (the x86-64/WSL image).

## Current release (arm64)

- **Version:** `v8-arm64`
- **URL:** https://github.com/adom-inc/hydrogen-lima-image/releases/download/v8/adom-golden-v8-arm64.tar.gz
- **SHA256:** `78b56e1781b445cac4aba4951074b1fbfb13c991e2a1db414572325e0aa72a02`

Pin all three in `hydrogen-app/src/runtime/macos.rs` (`TARBALL_URL`, `TARBALL_SHA256`,
`TARBALL_VERSION`).
...
- **Runtime = systemd-nspawn machine** `Adom-Workspace`, imported with
  ...
  Hydrogen-owned Lima utility VM `hydrogen-builder`. No WSL, **no `/etc/wsl.conf`** — nspawn
...
extensions, Hydrogen `shared`+`machine` skills, the wiki-managed adom skills
(`adom/hydrogen-mac-bootstrap`), the Adom wiki CLIs, adom-bridge CLI, the adom-wiki CLI, the
...
asset, and pushes a single-layer image to `ghcr.io/adom-inc/hydrogen-lima-image`.
