...
# rebuild). To preview a golden image, import it into a disposable Lima machine
# on the Mac and run code-server THERE (golden-image-test skill). This guard
...
if [[ "${ALLOW_PROOT_HERE:-}" != "1" ]]; then
    if pgrep -f code-server-entrypoint >/dev/null 2>&1 \
       ...
       || [[ -n "${VSCODE_PROXY_URI:-}" ]]; then
        ...
        echo "Preview the image in a disposable Lima machine instead (golden-image-test)." >&2
